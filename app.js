...
function pagesOf(item) {
    const pages = [];
    if (item.mindmap)
        pages.push({ src: item.mindmap, kind: "harta", scanIndex: 0 });
    item.images.forEach((src, scanIndex) =>
        pages.push({ src, kind: "scan", scanIndex }),
    );
    return pages;
}
...
function sameAuthor(item) {
    ...
    return STUDY_ITEMS.filter(
        (entry) => entry.author === item.author && entry.id !== item.id,
    );
}
...
function worksForCurrent(item) {
    ...
    return STUDY_ITEMS.filter((other) => {
        ...
        if (key.startsWith("modern"))
            return (
                movement.includes("modern") && !movement.includes("neomodern")
            );
...
function toggleReview(id) {
    ...
    progress.review = on
        ? progress.review.filter((x) => x !== id)
        : progress.review.concat(id);
...
function card(item) {
    ...
    return (
        ...
        (item.images.length
            ? item.images.length +
              (item.images.length === 1 ? " pagină" : " pagini")
            : "Fișă") +
...
function home() {
    ...
    const filtered = STUDY_ITEMS.filter((item) => {
        if (ui.category !== "all" && item.category !== ui.category)
            return false;
        if (ui.onlyMaps && !item.mindmap && !(item.blocks || []).length)
            return false;
    ...
    body +=
        ui.view === "programa"
            ? syllabus()
            : filtered.length
              ? '<ul class="cards">' + filtered.map(card).join("") + "</ul>"
              : '<p class="lead" style="text-align:center;padding:3rem 0">Niciun rezultat.</p>';
    return body;
}
...
function syllabus() {
    ...
    if (!rows.length)
        return '<p class="lead">Nicio operă din programă nu se potrivește.</p>';
    const groups = [];
    rows.forEach((row) => {
        ...
        groups
            .map((group) => {
                const items = group.rows
                    .map((row) => {
                        const inner =
                            ...
                            (row.studyId
                                ? "Deschide fișa"
                                : "Doar în programă, fără scanări aici") +
                            "</p>";
                        return (
                            "<li>" +
                            (row.studyId
                                ? '<a href="' +
                                  studyHref(row.studyId) +
                                  '">' +
                                  inner +
                                  "</a>"
                ...
                return (
                    ...
                    "</h2>" +
                    (group.footnote
                        ? '<p class="meta">' + esc(group.footnote) + "</p>"
                        : "") +
                    '<ul class="rows">' +
                    items +
                    "</ul></section>"
...
function study(item) {
    ...
    if (page) {
        ...
        media =
            ...
            esc(
                page.kind === "harta"
                    ? "Hartă mentală: " + item.title
                    : item.title + ", pagina " + (page.scanIndex + 1),
            ) +
            '"></button></div>' +
            (pages.length > 1
        ...
        media =
            '<p class="sheet">Fișa asta are doar punctele de mai jos, fără scanare.</p>';
    }
    ...
    let html =
        ...
        esc(
            [item.author, item.year, item.movement]
                .filter(Boolean)
                .join(" · ") || labelOf(item.category),
        ) +
        "</p>" +
        (item.species ? "<p>" + esc(item.species) + "</p>" : "") +
        "</div>" +
    ...
    if (item.remember)
        html +=
            '<p class="remember"><strong>De reținut: </strong>' +
            esc(item.remember) +
            "</p>";
    if (page)
        html +=
            '<a class="jump no-print" href="#material">Vezi harta sau paginile</a>';
    html +=
        '<div class="study"><section class="material" id="material">' +
        media +
        '</section><aside class="notes">' +
        (blocks ||
            '<p class="sheet">Pentru opera asta ai comentariul scanat. Deschide paginile și mărește-le ca să citești liniile.</p>') +
        (item.conclusion
            ? '<p class="sheet">' + esc(item.conclusion) + "</p>"
            : "") +
        (item.source
    ...
    if (related.length)
        html +=
            "<h2>Opere din " +
            esc(item.title.toLowerCase()) +
            '</h2><ul class="links">' +
            related.map(workLink).join("") +
            "</ul>";
    if (authorWorks.length)
        html +=
            ...
            esc(item.author) +
            '</h2><ul class="links">' +
            authorWorks.map(workLink).join("") +
            "</ul>";
    html += '<nav class="sibs no-print">';
    html += nav.prev
        ? '<a href="' +
          studyHref(nav.prev.id) +
          '"><span class="meta">Anterior</span><strong style="display:block;font-family:var(--display);font-size:1.15rem">' +
          esc(nav.prev.title) +
          "</strong></a>"
        : "<span></span>";
    if (nav.next) {
...
function zoom(item, page, pageLabel, canStep) {
    ...
    return (
        ...
        '" style="' +
        (wide
            ? "width:" + ui.scale * 90 + "vw;max-width:none;max-height:none"
            : "") +
        '"></div>' +
        (canStep
...
function recap() {
    const pool = STUDY_ITEMS.filter(
        (item) => ui.recapCat === "all" || item.category === ui.recapCat,
    );
    ...
    const knownCount = STUDY_ITEMS.filter((entry) =>
        progress.known.includes(entry.id),
    ).length;
    const pills = CATEGORIES.map(
        (cat) =>
            '<button class="pill' +
    ...
    let cardHtml =
        '<p class="lead" style="text-align:center">Nimic în categoria asta.</p>';
    if (item) {
        cardHtml =
            '<article class="flash"><p class="meta">' +
            esc(
                [item.author, item.year, item.species]
                    .filter(Boolean)
                    .join(" · ") ||
                    item.movement ||
                    "",
            ...
            (ui.recapShown
                ? '<div style="margin-top:1rem;font-size:1.15rem;line-height:1.5">' +
                  ...
                  (item.conclusion
                      ...
                      : "") +
                  (item.movement
                      ? '<p class="meta">' + esc(item.movement) + "</p>"
                      : "") +
                  "</div>"
...
function render(scroll) {
    ...
    if (current.name === "recap") {
        ...
        if (!item) {
            ...
            body =
                '<div class="wrap" style="text-align:center;padding:4rem 1rem"><h1>Fișa nu există</h1><p><a href="#/">Toate fișele</a></p></div>';
        } else {
    ...
    if (current.name === "study") {
        ...
        if (item) {
            ...
            window.setTimeout(
                () => pages.forEach((page) => warm(page.src)),
                300,
            );
        }
    }
...
function paintPage() {
    ...
    document
        .querySelectorAll("[data-act=step][data-id='-1']")
        .forEach((button) => {
...
document.addEventListener("click", (event) => {
    ...
    if (act === "theme") {
        ...
        ui.scale = Math.min(
            4,
            Math.max(1, ui.scale + Number(el.dataset.id) * 0.25),
        );
        render(false);
